...
**Tech Stack:** TypeScript (strict, ESM, NodeNext), Vitest, zod, Express, SQLite via Node's built-in `node:sqlite` (see Task 11 note — swapped from `better-sqlite3` when native compilation was unavailable in the build environment), React + Vite.
...
- Every scoring bucket table is a half-open interval `[min, next_min)` — a value exactly at a threshold belongs to the bucket starting there, matching `bucketScore`'s actual mechanism. The point *values* in every Task 6/7 bucket table are correct as written; if a reviewer flags a boundary against the design spec, check the spec's §5 boundary-convention note before assuming the code is wrong (see ledger: Task 6 fix round 1).
...
(`passWithNoTests` is required for Step 8's "exits 0 with no test files" to actually hold — vitest 2.x exits 1 on an empty suite by default.)
...
- Produces: `decide(raw: unknown): Decision` — the engine's single public entrypoint, consumed by `api` (Task 11) and `packages/engine/src/fixtures/personas.test.ts` (Task 10; personas are hand-written `RawApplicant` fixtures run through `decide()` directly in the test file — there is no separate `fixtures/generate.ts` script).
...
**Note:** the `"test": "vitest run"` script and `"vitest": "^2.0.5"` devDependency shown above did not survive Task 11's own fix round — see the Vitest deviation note below for what actually shipped (`"test": "tsx --test src/**/*.test.ts"`, no `vitest` dependency).

**Deviation from the original plan (resolved during Task 11):** the plan originally specified `better-sqlite3`, a native addon requiring a C++ toolchain to compile. That build failed in the actual implementation environment (no Visual Studio C++ workload installed, and no prebuilt binary exists for this machine's Node version). Resolution: use Node's own built-in `node:sqlite` module instead — same SQLite engine, zero native compilation, zero extra dependency. Verified API-compatible with the design below (named `@param` binding, `.get()`/`.all()`/`.run()` all work identically to better-sqlite3 for this file's usage). `node:sqlite` is still experimental in Node 22/24 and prints one `ExperimentalWarning` to stderr on first use — this is expected and does not indicate a test failure or code defect; do not attempt to suppress or "fix" it.

**Deviation from the original plan (resolved during Task 11's own fix round, commit 13504f8):** the `vitest` devDependency and `packages/api/vitest.config.ts` specified below were dropped from this one package. Vitest runs through Vite's transform/bundling pipeline, which cannot resolve the `node:sqlite` builtin used by `db.ts` — any test file that imports it fails under Vitest. Resolution: `packages/api` uses Node's built-in `node:test`/`node:assert` instead (`"test": "tsx --test src/**/*.test.ts"`), which runs directly under Node with no bundling step. `shared`, `engine`, and `ui` are unaffected and still use Vitest. See the actual shipped test file `packages/api/src/repository.test.ts` for the real pattern (`import { test } from 'node:test'; import { strict as assert } from 'node:assert';`), not the `vitest`-flavored snippet in Step 4 below.
...
- [ ] **Step 3: ~~Add `packages/api/vitest.config.ts`~~ Not added — see deviation note above; this package uses `node:test` instead of Vitest.
